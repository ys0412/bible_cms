<template>
    <div>
        <el-button type="primary" @click="openDialog()">添加书籍</el-button>

        <el-table :data="books" border>
            <el-table-column prop="bookId" label="ID" width="50" />
            <el-table-column prop="bookName" label="书名">
                <template #default="{ row }">
                    <el-link type="primary" @click="goToBookDetail(row.bookId)">{{ row.bookName }}</el-link>
                </template>
            </el-table-column>
...
export default {
    setup() {
        ...
        const handleFileChange = (file) => {
            ...
            console.log("本地图片 URL:", book.value.coverUrl);
        };
        ...
        const saveBook = async () => {
            try {
                ...
                if (selectedFile.value) {
                    ...
                    const res = await axios.post(BASE_URL + "/upload/bookCover", formData, {
        ...
        return { books, book, dialogVisible, openDialog, saveBook, deleteBook, handleFileChange, beforeUpload, getCoverUrl, goToBookDetail, statusText };
    }
};
